    ...
    <div class="panel-body">
      ...
      <template v-if="config.module === 'chinese'">
        <div class="panel-block">
          ...
          <div class="chips">
            <button
              v-for="g in gridTypes" :key="g.value"
              class="chip" :class="{ active: config.gridType === g.value }"
        ...
        <div class="panel-block">
          ...
          <div class="chips">
            <button
              v-for="f in fontStyles" :key="f.value"
              class="chip" :class="{ active: config.fontStyle === f.value }"
        ...
        <div class="panel-block">
          ...
          <div class="swatches">
            <button
              v-for="c in charColors" :key="c.value"
              class="swatch" :class="{ active: config.color === c.value }"
              :style="{ '--sw': c.hex }" :title="c.label"
          ...
          <label class="toggle-row">
            ...
            <span class="toggle-wrap">
              <input
                ...
                @change="updateConfig({ showPinyin: ($event.target as HTMLInputElement).checked })"
              />
        ...
        <div class="panel-block">
          ...
          <div class="field-row">
            <label class="field">
              ...
              <input class="num-input" type="number" min="0" max="5"
                :value="config.traceCount"
                @input="updateConfig({ traceCount: +($event.target as HTMLInputElement).value })"
              />
            </label>
            <label class="field">
              <span class="field-label">练习格数</span>
              <input class="num-input" type="number" min="0" max="10"
                :value="config.practiceCount"
                @input="updateConfig({ practiceCount: +($event.target as HTMLInputElement).value })"
              />
            </label>
          </div>
        </div>
        ...
        <div class="panel-block">
          ...
          <input type="color" class="color-picker"
            ...
            @input="updateConfig({ gridColor: ($event.target as HTMLInputElement).value })"
          />
        </div>
      </template>
      ...
      <template v-else-if="config.module === 'stroke'">
        <div class="panel-block">
          ...
          <div class="check-grid">
            <label
              v-for="sp in strokePatternOptions" :key="sp.value"
              class="check-item" :class="{ active: config.strokePatterns.includes(sp.value) }"
            >
              <input
                type="checkbox"
        ...
        <div class="panel-block">
          ...
          <div class="chips">
            <button
              ...
              class="chip" :class="{ active: config.strokeDifficulty === d.value }"
      ...
      <!-- ════ 语文 · 拼音默写 ════ -->
      <template v-else-if="config.module === 'dictation' && config.subject === 'yuwen'">
        <div class="panel-block">
          <div class="block-title">默写模式</div>
          <div class="chips wrap">
            <button
              v-for="dm in yuwenDictModes" :key="dm.value"
              class="chip" :class="{ active: config.dictationMode === dm.value }"
          ...
          <div class="field-row">
            <label class="field">
              ...
              <input class="num-input" type="number" min="0" max="100"
                ...
                @input="updateConfig({ numberRange: [+($event.target as HTMLInputElement).value, config.numberRange[1]] })"
              />
            </label>
            <label class="field">
              <span class="field-label">结束</span>
              <input class="num-input" type="number" min="1" max="100"
                :value="config.numberRange[1]"
                @input="updateConfig({ numberRange: [config.numberRange[0], +($event.target as HTMLInputElement).value] })"
              />
            </label>
          </div>
        </div>
          ...
          <div class="field-row">
            <label class="field">
              ...
              <input class="num-input" type="number" min="0" max="20"
                ...
                @input="updateConfig({ mathCount: +($event.target as HTMLInputElement).value })"
              />
            </label>
            <label class="field">
              <span class="field-label">最大数</span>
              <input class="num-input" type="number" min="5" max="100"
                :value="config.mathMax"
                @input="updateConfig({ mathMax: +($event.target as HTMLInputElement).value })"
              />
            </label>
          </div>
        </div>
      ...
      <template v-else-if="config.module === 'english'">
        <div class="panel-block">
          ...
          <div class="chips">
            <button
              v-for="ef in englishFonts" :key="ef.value"
              class="chip" :class="{ active: config.englishFont === ef.value }"
      ...
      <!-- ════ 英语 · 英文默写 ════ -->
      <template v-else-if="config.module === 'dictation' && config.subject === 'yingyu'">
        <div class="panel-block">
          <div class="block-title">提示方式</div>
          <div class="chips">
            <button
              v-for="dm in yingyuDictModes" :key="dm.value"
              class="chip" :class="{ active: config.dictationMode === dm.value }"
              @click="updateConfig({ dictationMode: dm.value })"
            >{{ dm.label }}</button>
          </div>
          ...
          <div class="field-row" style="align-items:center;gap:8px">
            <label class="field">
              ...
              <div class="row-col-stepper">
                <button class="step-btn"
                  ...
                  @click="updateConfig({ hintFontSize: config.hintFontSize - 1 })">−</button>
                <span class="step-val" style="min-width:32px">{{ config.hintFontSize }}px</span>
                <button class="step-btn"
            ...
            <label class="field">
              ...
              <input type="color" class="color-picker"
                ...
                @input="updateConfig({ hintColor: ($event.target as HTMLInputElement).value })"
              />
            </label>
          </div>
          <label class="toggle-row" style="margin-top:6px">
            ...
            <span class="toggle-wrap">
              <input type="checkbox" class="toggle-input"
                ...
                @change="updateConfig({ hintBold: ($event.target as HTMLInputElement).checked })"
              />
              <span class="toggle-track" />
            </span>
          </label>
          ...
          <label class="toggle-row">
            ...
            <span class="toggle-wrap">
              <input type="checkbox" class="toggle-input"
                ...
                @change="updateConfig({ showCorrection: ($event.target as HTMLInputElement).checked })"
              />
              <span class="toggle-track" />
            </span>
          </label>
        ...
        <div class="panel-block">
            ...
            <label class="field">
              ...
              <div class="row-col-stepper">
                <button class="step-btn"
                  ...
                  @click="updateConfig({ gridScale: Math.round((config.gridScale - 0.1) * 10) / 10 })">−</button>
                <span class="step-val" style="min-width:36px">{{ config.gridScale.toFixed(1) }}x</span>
                <button class="step-btn"
          ...
          <div class="field-row" style="margin-top:8px">
            ...
            <div class="chips" style="margin-top:0">
              <button class="chip" :class="{ active: config.gridLineStyle === 'dashed' }"
                @click="updateConfig({ gridLineStyle: 'dashed' })">虚线</button>
              <button class="chip" :class="{ active: config.gridLineStyle === 'solid' }"
                @click="updateConfig({ gridLineStyle: 'solid' })">实线</button>
            </div>
          </div>
          <div class="grid-colors-row">
            <label class="grid-color-item" title="顶线 / 底线">
              <span class="grid-color-dot" :style="{ background: config.gridTopColor }" />
              <span class="grid-color-label">顶/底</span>
              <input type="color" class="color-picker-inline"
                :value="config.gridTopColor"
                @input="updateConfig({ gridTopColor: ($event.target as HTMLInputElement).value })"
              />
            </label>
            <label class="grid-color-item" title="第二辅助线">
              <span class="grid-color-dot" :style="{ background: config.gridMidColor }" />
              <span class="grid-color-label">辅线</span>
              <input type="color" class="color-picker-inline"
                :value="config.gridMidColor"
                @input="updateConfig({ gridMidColor: ($event.target as HTMLInputElement).value })"
              />
            </label>
            <label class="grid-color-item" title="基线（第三线）">
              <span class="grid-color-dot" :style="{ background: config.gridBaseColor }" />
              <span class="grid-color-label">基线</span>
              <input type="color" class="color-picker-inline"
                :value="config.gridBaseColor"
                @input="updateConfig({ gridBaseColor: ($event.target as HTMLInputElement).value })"
              />
            </label>
          </div>
        </div>
...
import type {
  WorksheetConfig, PresetTemplate, StrokePattern,
  Subject, ModuleType, DictationSubMode,
...
// ── Theme ──
const subjectThemes: Record<Subject, { color: string; bg: string; light: string }> = {
...
const subjectModules: Record<Subject, ModuleItem[]> = {
  yuwen: [
    ...
    { uid: 'dict-yuwen',  id: 'dictation', icon: '默', label: '拼音默写', dictMode: 'pinyin-only' },
  ],
  shuxue: [
    { uid: 'number', id: 'number', icon: '123', label: '数字书写' },
  ],
  yingyu: [
    { uid: 'english',      id: 'english',   icon: 'ABC', label: '单词书写' },
    { uid: 'dict-yingyu',  id: 'dictation', icon: '写',  label: '英文默写', dictMode: 'chinese-hint' },
  ],
}

const currentModules = computed(() => subjectModules[props.config.subject] ?? [])
...
const contentLabel = computed(() => {
  switch (props.config.module) {
    case 'chinese': return '练习汉字'
    case 'english': return '单词 / 短语（逗号分隔）'
    case 'dictation':
...
const contentPlaceholder = computed(() => {
  switch (props.config.module) {
    case 'chinese':   return '输入要练习的汉字，如：床前明月光'
    case 'english':   return '如：Apple, Banana, Good morning'
    case 'dictation':
...
function setRowCol(index: number, val: number) {
  ...
  cols[index] = Math.max(1, Math.min(6, val))
...
.brand-mark {
  ...
  height: 36px;
  border-radius: 10px;
  background: linear-gradient(135deg, var(--t-color), color-mix(in srgb, var(--t-color) 55%, #000));
  display: flex;
  align-items: center;
...
.panel-body::-webkit-scrollbar-thumb { background: #dde2e8; border-radius: 2px; }
...
.swatch:hover { transform: scale(1.18); box-shadow: 0 3px 8px rgba(0,0,0,0.2); }
.swatch.active {
  border-color: var(--t-color);
...
.tr-label { font-size: 13px; font-weight: 500; color: #3d4451; }
...
.toggle-input:checked + .toggle-track { background: var(--t-color); }
.toggle-input:checked + .toggle-track::after { transform: translateX(17px); }

/* ── Fields ── */
.field-row { display: flex; gap: 8px; }
.field { flex: 1; display: flex; flex-direction: column; gap: 5px; }
.field-label { font-size: 11px; font-weight: 500; color: #adb5bd; }
.num-input {
  width: 100%;
  padding: 7px 8px;
  border: 1.5px solid #eaeef3;
...
.color-picker:hover { border-color: var(--t-color); }
...
.check-item input { accent-color: var(--t-color); cursor: pointer; width: 13px; height: 13px; }
.check-item.active { border-color: var(--t-color); background: var(--t-bg); color: var(--t-color); }
...
.add-row-btn:hover { background: var(--t-color); color: white; }

.row-cols-list { display: flex; flex-direction: column; gap: 5px; }
...
.row-col-item:hover { border-color: var(--t-color); }
...
.step-btn:hover:not(:disabled) { background: var(--t-color); color: white; }
.step-btn:disabled { color: #c8d0da; cursor: not-allowed; }
...
.row-del-btn:hover:not(:disabled) { background: #fee2e2; color: #e03030; }
...
.btn-print:active { transform: translateY(0); filter: brightness(0.96); }
</style>
